...
- **Shared adapter helpers go in an `internal/` package named for the concern it serves, never duplicated per adapter and never in `internal/domain/`.** E.g. `internal/httpkit` (transport), `internal/typeutil` (decoding); logic scoped to one adapter family nests under that family instead, e.g. `internal/scm/scmcore`.
