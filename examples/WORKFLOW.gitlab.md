...
{{/* Sortie sample workflow, GitLab + Claude Code.
     Required env vars:
       SORTIE_GITLAB_TOKEN    GitLab access token (personal, project, or group), api scope
       SORTIE_GITLAB_PROJECT  Target project as group/project (e.g. group/project) or
                              its numeric project id
       SORTIE_REPO_URL        Git clone URL for the repository
     Optional:
       SORTIE_GITLAB_ENDPOINT Self-hosted instance base URL (e.g. https://gitlab.example.com);
                              leave unset for GitLab.com, which the adapter uses by default
