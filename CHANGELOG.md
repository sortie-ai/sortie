...
- The new `agent-client-protocol` agent kind runs Agent Client Protocol-compatible runtimes over stdio and resumes previous sessions when supported by the runtime. Locally launched runtimes can use workflow-configured MCP servers, while requests requiring human input are refused or end the attempt rather than waiting indefinitely. Token-based budgets do not apply because the protocol does not report token usage; when the runtime itself ends a turn at its own token limit the claim is released rather than retried into the same limit, while a turn that exhausts the runtime's request or turn budget is retried on a fresh session. Stopping a session now closes it through the protocol first when the runtime advertised that capability at handshake, instead of always ending it with a process signal alone.
