...
- The `codex` agent kind now reports what the agent runtime wrote to its standard error, so a session that fails during the handshake, or a turn that ends because the runtime stopped producing output, carries the runtime's own error message rather than an exit status alone. Across every agent kind that launches a local runtime, standard error captured before collection has to be cut short is now reported alongside the notice that it is incomplete, instead of being replaced by it.
