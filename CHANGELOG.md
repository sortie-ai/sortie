...
- Agents that Sortie starts on a remote host through `worker.ssh_hosts` can now receive environment variables from Sortie's own environment: list their names under the new `worker.ssh_pass_env` setting. A variable named under the new `worker.ssh_disallow_pass_env` setting is never sent to a remote host, so the value or login the host holds stays in effect.
