...
// errors.
//
// HTTP 405 (method not allowed) and HTTP 409 (conflict) responses
// classified by the transport as ErrTrackerAPI are remapped to
// ErrSCMConflict. The transport's classifyHTTPError preserves GitHub's
// response body in the message, so the already-merged subcase the
// SCMAdapter contract requires is signaled by the substring "already
// merged" that GitHub returns for duplicate merge attempts; head-SHA
// drift and branch-protection refusals reach the caller through the
// same kind without that marker.
//
// The 405/409 promotion is suppressed when the message prefix matches
// "POST /graphql:". The GraphQL read surface does not carry
// duplicate-merge semantics, so errors originating from the GraphQL
// endpoint pass through with the default ErrSCMAPI kind.
func toSCMError(err error) *domain.SCMError {
	...
	// Promote 405 method-not-allowed and 409 conflict responses to
	// ErrSCMConflict. The transport classifier records these as
	// ErrTrackerAPI with a stable message prefix ("method not allowed"
	// or "conflict") generated by classifyHTTPError. GraphQL-originated
	// errors carry the literal "POST /graphql:" prefix and are exempt
	// from the promotion: the read surface has no duplicate-merge
	// disposition.
	if scmKind == domain.ErrSCMAPI && !strings.HasPrefix(te.Message, "POST /graphql:") {
