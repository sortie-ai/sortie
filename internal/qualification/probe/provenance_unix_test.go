...
func firstProfile(t *testing.T) qualification.RuntimeProfile {
	t.Helper()
	paths := stalenessProfilePaths(t)
	profile, err := qualification.ReadRuntimeProfileFile(paths[0])
	if err != nil {
		t.Fatalf("ReadRuntimeProfileFile(%s) error = %v, want nil", paths[0], err)
	}
	return profile
}
...
func identityRecordFor(t *testing.T, records []qualification.Record, sessionID string) qualification.Record {
	t.Helper()
	for _, rec := range records {
		if rec.Scenario == qualification.ScenarioRuntimeIdentity && rec.SessionID != nil && *rec.SessionID == sessionID {
			return rec
		}
	}
...
func TestPublishedEvidenceReplaysToTheSameGrades(t *testing.T) {
	...
	for i, rec := range published {
		if !qualification.RecordsEqual(rec, fixture.Records[i]) {
	...
	if _, err := ConclusionsFromRecords(published, verdict, profile); err != nil {
		t.Fatalf("derive the summary from the published evidence: %v", err)
	}
}
...
func TestCollectionProvenanceNamesTheRunWithoutCredentials(t *testing.T) {
	...
	identities := map[string]qualification.SessionIdentity{"sess-a": {Name: "sample-runtime", Version: "1.2.3"}}
	...
	if provenance.ProfileDigest != profile.Digest() {
		t.Errorf("provenance profile_digest = %s, want %s", provenance.ProfileDigest, profile.Digest())
	}
