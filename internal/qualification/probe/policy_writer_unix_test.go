...
func policyRequestingProfile(runtimeID, toolNameFormat, policyFormat string) qualification.RuntimeProfile {
	return qualification.RuntimeProfile{
		...
		EntryPoints: map[qualification.Surface]qualification.EntryPoint{
			qualification.SurfaceProtocol: {Args: []string{"--policy", policyPlaceholder, "--acp"}},
		},
	}
}
...
func TestWriteToolServerPolicyWritesOnlyForARuntimeThatAsksAndIsUnderstood(t *testing.T) {
	...
	t.Run("a profile that asks for no policy file gets none written", func(t *testing.T) {
		...
		if path := writeToolServerPolicy(t, dir, profile); path != "" {
	...
	t.Run("a runtime that asks for a policy file in no known format is refused", func(t *testing.T) {
		...
		profile := policyRequestingProfile("runtime-with-no-writer", "{tool}@{server}", "format-with-no-writer")

		write, err := toolPolicyWriterFor(profile)

		if err == nil {
			t.Fatalf("toolPolicyWriterFor(%q) error = nil, want a refusal: substituting a tool name into another runtime's file format produces a file that runtime cannot read", profile.RuntimeID)
		}
		if write != nil {
	...
	t.Run("the written rule names the tool under the runtime's own format", func(t *testing.T) {
		...
		dir := t.TempDir()
		path := writeToolServerPolicy(t, dir, policyRequestingProfile("sample-runtime", "mcp_{server}_{tool}", qualification.ToolPolicyFormatTOMLRuleList))
...
func TestRequestsToolPolicy(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			profile: policyRequestingProfile("sample-runtime", "mcp_{server}_{tool}", qualification.ToolPolicyFormatTOMLRuleList),
			want:    true,
		},
		{
