...
func TestDeclarableSurfaceUnproducedRejectsARuntimeFailure(t *testing.T) {
	...
	measured := qualification.DeclarableSurfaces
	caseID := qualification.CaseLimitReached

	byRuntimeFailure := collectedObservations{semantic: map[qualification.Surface]map[qualification.Case]qualification.Observation{}}
	byInductionMiss := collectedObservations{semantic: map[qualification.Surface]map[qualification.Case]qualification.Observation{}}
	for _, surface := range measured {
		byRuntimeFailure.semantic[surface] = map[qualification.Case]qualification.Observation{
			caseID: {
		...
		byInductionMiss.semantic[surface] = map[qualification.Case]qualification.Observation{
			caseID: {
				Grade:     qualification.GradeNotObserved,
