...
func TestProtocolTokenInventoryNeverProvesAZeroFromAnUnreadTransport(t *testing.T) {
	...
	if inventory.Grade != qualification.GradeNotObserved {
		t.Errorf("protocolTokenInventory(...).Grade = %s, want %s: an unread transport is an unknown, never a measured absence",
			inventory.Grade, qualification.GradeNotObserved)
	}
}
...
func TestRawExtensionAccountKeepsTheFourAnswersApart(t *testing.T) {
	...
	if len([]rune(account)) > qualification.DetailBound {
		t.Errorf("account() is %d runes, want at most %d: a longer one is truncated in the published record", len([]rune(account)), qualification.DetailBound)
	}
}
