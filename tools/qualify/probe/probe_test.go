...
// sampleProfileJSON's notes_path, measurement_path, and published_sample
// name real tracked files under the checkout root: a fixture cannot fake
// one out from under ResolveCoordinates.
const sampleProfileJSON = `{
  "schema_version": 4,
  "runtime_id": "sample-runtime",
  "identity_tokens": ["sample"],
  "notes_path": "docs/gemini-adapter-notes.md",
  "measurement_path": "tools/qualify/probe/testdata/gemini-cli/measurement.json",
  "published_sample": "examples/WORKFLOW.agent-client-protocol.md",
...
func writeValidProfileFixture(t *testing.T) string {
	t.Helper()
	profilePath := filepath.Join(t.TempDir(), "sample-runtime.json")
