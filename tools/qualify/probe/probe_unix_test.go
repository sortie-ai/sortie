...
type semanticOrchestrationParams struct {
	// ReceiptDir, when non-empty, proves the cancellation branch never ran,
	// which a declared not-inducible pair's Observation alone cannot.
	ReceiptDir string
}
	...
	p := profile.RuntimeProfile{
		...
		Recognizers: map[evidence.Surface]profile.Recognizer{
			evidence.SurfaceNativeJSON: {
				...
				StatusMember:  "status",
				StatusCases:   map[string]evidence.Case{"cancelled": evidence.CaseCancellation, "refusal": evidence.CaseRuntimeRefusal},
				StatusEndTurn: []string{"end_turn"},
			},
		},
	}
	if notInducible {
		p.NotInducibleCases = []profile.SurfaceNotInducible{
			{Surface: evidence.SurfaceNativeJSON, Case: evidence.CaseCancellation, Reason: evidence.NotInducibleTerminalAtExitOnly},
		}
	}
	return Coordinates{CommandPath: runtimePath, Profile: p}
}
...
func TestInduceNativeSemanticsSkipsDeclaredNotInducibleCancellation(t *testing.T) {
	...
	t.Run("a declared not-inducible pair produces zero cancellation launches", func(t *testing.T) {
		...
		induceNativeSemantics(t, coords, newFixture(t), collected, evidence.SurfaceNativeJSON)
		...
		if obs, ok := collected.semantic[evidence.SurfaceNativeJSON][evidence.CaseCancellation]; ok {
	...
	t.Run("no declaration launches the cancellation probe", func(t *testing.T) {
		...
		if _, ok := collected.semantic[evidence.SurfaceNativeJSON][evidence.CaseCancellation]; !ok {
...
func TestLaunchNativeProbe(t *testing.T) {
	...
	t.Run("kills the whole process group, not just its direct child", func(t *testing.T) {
		...
		// Nil Stdout/Stderr routes to the null device rather than the leader's
		// piped streams: an inherited pipe would stall cmd.Wait as long as the
		// hanging grandchild holds its write end open.
...
func corroborateAbsentSurfaceCoordinates(t *testing.T, runtimePath string) Coordinates {
	...
	if err != nil {
		t.Fatalf("profile.Load(%q) error = %v, want nil", profilePath, err)
	}
	return Coordinates{CommandPath: runtimePath, Model: "fixture-model", Profile: p}
}
...
func TestCorroborateAbsentSurface(t *testing.T) {
	...
	corroborateAbsentSurface(t, coords, testSharedFixture(t), evidence.SurfaceNativeJSON)
}
