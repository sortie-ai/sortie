...
func TestResolveSharedFixtureFailsOnStrayProjectConfigPath(t *testing.T) {
	...
	if os.Getenv("PROBE_RESOLVE_SHARED_FIXTURE_HELPER_PROCESS") == "1" {
		coords := Coordinates{Profile: profile.RuntimeProfile{ProjectConfigPaths: []string{".git/config"}}}
...
// awaitProbeStarted attests only to a file the measured runtime can write
// itself; no induction binds a signal to it.
