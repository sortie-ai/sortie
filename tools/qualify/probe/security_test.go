...
func TestInduceWorkspaceSecurity(t *testing.T) {
	...
	t.Run("a leaked project_config_paths entry grades not_observed with runtime_failed", func(t *testing.T) {
		...
		coords := Coordinates{Profile: profile.RuntimeProfile{ProjectConfigPaths: []string{".mcp.json"}}}
		fixture := &sharedFixture{workspaceRoot: workspaceRoot, tracker: &groupTracker{}}

		obs := induceWorkspaceSecurity(t, coords, fixture)

		if obs.Grade != evidence.GradeNotObserved || obs.Outcome != evidence.OutcomeRuntimeFailed {
			t.Errorf("induceWorkspaceSecurity(...) = %+v, want grade %q and outcome %q", obs, evidence.GradeNotObserved, evidence.OutcomeRuntimeFailed)
		}
	})
	...
	t.Run("no observable process-group member grades not_observed with fixture_induction_failed", func(t *testing.T) {
		...
		if obs.Grade != evidence.GradeNotObserved || obs.Outcome != evidence.OutcomeFixtureInductionFailed {
			t.Errorf("induceWorkspaceSecurity(...) = %+v, want grade %q and outcome %q: process provenance cannot be claimed over an empty subject", obs, evidence.GradeNotObserved, evidence.OutcomeFixtureInductionFailed)
		}
	})
	...
	t.Run("a live group of the launched command grades usable with pass", func(t *testing.T) {
		...
		coords := Coordinates{CommandPath: commandPath, Profile: profile.RuntimeProfile{ProjectConfigPaths: []string{".mcp.json"}}}
		...
		if obs.Grade != evidence.GradeUsable || obs.Outcome != evidence.OutcomePass {
			t.Errorf("induceWorkspaceSecurity(...) = %+v, want grade %q and outcome %q", obs, evidence.GradeUsable, evidence.OutcomePass)
		}
...
func TestInduceProcessCleanupReportsALiveGroupAsALeak(t *testing.T) {
	...
	if obs.Grade == evidence.GradeUsable {
		t.Errorf("induceProcessCleanup(...) = %+v, want a negative observation for a group that was alive when the run was graded", obs)
	}
	if present, err := procgroup.Present(pgid); err != nil || present {
		t.Errorf("procgroup.Present(%d) = %v, %v, want false, nil: a failing observation must still leave no child behind", pgid, present, err)
	}
}
...
func TestInduceProcessCleanupStopsRegisteredSessionsBeforeReading(t *testing.T) {
	...
	t.Run("a session whose group survives its own stop is a leak the drain still clears", func(t *testing.T) {
		...
		if present, err := procgroup.Present(pgid); err != nil || present {
			t.Errorf("procgroup.Present(%d) = %v, %v, want false, nil after the drain", pgid, present, err)
		}
	})
	...
	t.Run("a session its own stop ended grades usable with pass", func(t *testing.T) {
		...
		if obs.Grade != evidence.GradeUsable || obs.Outcome != evidence.OutcomePass {
			t.Errorf("induceProcessCleanup(...) = %+v, want grade %q and outcome %q", obs, evidence.GradeUsable, evidence.OutcomePass)
		}
...
func TestInduceWorkspaceSecurityNoticesAConfigInTheLaunchDirectory(t *testing.T) {
	...
	coords := Coordinates{Profile: profile.RuntimeProfile{ProjectConfigPaths: []string{".runtime-config"}}}
...
func TestInduceWorkspaceSecurityGradesAnInterpretedLaunch(t *testing.T) {
	...
	if obs.Grade != evidence.GradeUsable || obs.Outcome != evidence.OutcomePass {
		t.Errorf("induceWorkspaceSecurity(...) = %+v, want grade %q and outcome %q: the launch is the one the run started, whatever name its interpreter left in the process table", obs, evidence.GradeUsable, evidence.OutcomePass)
	}
}
...
// TestDetachedLeakHelperProcess detaches a sleeper into its own session, the
// shape a runtime's shell tool leaves behind.
...
// awaitReparentedDescendant waits inside the session's stop so the control
// grades the case it names rather than racing it.
