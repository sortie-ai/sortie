// Package probe drives the live qualification profile against any runtime
// a [profile.RuntimeProfile] describes, gated behind
// SORTIE_CLIENTPROTOCOL_QUALIFICATION_TEST. Every graded observation is
// delegated to tools/qualify/eval, so the live and offline paths share one
// derivation. Use [Gated] to resolve coordinates and [Run] to drive one
// collection.
package probe
...
// Coordinates is the resolved coordinate set a live profile launches with.
// It carries names and the decoded profile only: no credential value ever
// travels through it.
type Coordinates struct {
	// CommandPath is the resolved executable path. Surface-specific flags
	// are appended by the launchers, never stored here.
	CommandPath string
...
// Result is what one Run call produced, naming every artifact the run wrote
// so a caller reads provenance from the result rather than the run's log
// lines.
type Result struct {
...
// permission. Windows executability is decided by the loader, not a mode
// bit, so the check applies only where the bit exists.
...
// parseCommand resolves exactly one executable path with no arguments. A
// bare name resolves through PATH; a path with a separator resolves
// directly.
...
// parseAuthEnvNames parses the comma-separated authentication variable
// names. An empty list declares a stored-login runtime; an absent
// coordinate still fails, never passing silently as that declaration.
...
// coordinateValue reads one coordinate. An absent coordinate resolves to the
// empty string, which each coordinate's own rule rejects.
...
// absent or empty, and ("", false) when every name has a non-empty value.
...
// ResolveCoordinates resolves the gate-enabled coordinates from env: an
// executable path, model, authentication names, and a runtime profile.
// An unsupplied authentication value is [Gated]'s clean skip, not a
// failure here.
func ResolveCoordinates(env func(string) (string, bool)) (Coordinates, error) {
	...
	if err != nil {
		return Coordinates{}, fmt.Errorf("resolve the checkout root for %s: %w", qualificationProfileEnv, err)
	}
	runtimeProfile, err := profile.Load(root, profilePath)
	if err != nil {
...
// it resolves coordinates, calling t.Fatalf on any invalid one and
// skipping cleanly on an unsupplied authentication credential.
