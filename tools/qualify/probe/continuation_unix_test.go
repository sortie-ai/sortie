...
func TestInduceNativeContinuationTransportLoss(t *testing.T) {
	...
	if seed.obs.Grade != evidence.GradeNotObserved || seed.obs.Outcome != evidence.OutcomeRuntimeFailed {
		t.Errorf("induceNativeContinuation(...) seed = %+v, want not_observed/runtime_failed: a launch the collector itself ended must never be graded usable, and never a pass", seed.obs)
	}

	if recall.obs.Grade != evidence.GradeNotObserved || recall.obs.Outcome != evidence.OutcomeRuntimeFailed || recall.obs.Detail != evidence.RecallUnobservedActual {
		t.Errorf("induceNativeContinuation(...) recall = %+v, want not_observed/runtime_failed/%q: the validator requires that closed detail for an unobserved actual session, never a gap-level pass with the fresh-fallback detail", recall.obs, evidence.RecallUnobservedActual)
	}
}
...
func TestInduceNativeContinuationArgvAndWorkspace(t *testing.T) {
	...
	induceNativeContinuation(t, Coordinates{CommandPath: script, Profile: p}, fixture, semanticTestSurface)
