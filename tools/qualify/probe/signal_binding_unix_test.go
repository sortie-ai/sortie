...
func TestAsyncSignalBindsToObservedProbeExecution(t *testing.T) {
	...
	t.Run("a runtime that forges the marker without running the probe induces nothing", func(t *testing.T) {
		...
		if g.obs.Grade != evidence.GradeNotObserved || g.obs.Outcome != evidence.OutcomeFixtureInductionFailed {
			t.Errorf("induceRetryableTransport(...) = %+v, want not_observed/fixture_induction_failed: no probe ever ran, so no signal moment was earned", g.obs)
		}
		if g.obs.EvidencePath != "" {
			t.Errorf("induceRetryableTransport(...) evidence path = %q, want empty", g.obs.EvidencePath)
		}
	})
	...
	t.Run("a runtime that really runs the probe earns the signal", func(t *testing.T) {
		...
		if g.obs.Grade != evidence.GradeUsable || g.obs.Outcome != evidence.OutcomePass {
			t.Errorf("induceRetryableTransport(...) = %+v, want usable/pass: the probe really ran under this launch", g.obs)
		}
	})
}
...
func TestNativeAsyncSignalBoundsTheWaitAfterTheSignal(t *testing.T) {
	...
	started := time.Now()
	g := induceCancellation(t, semanticCoordinates(script), fixture, semanticTestSurface)
	...
	if g.obs.Grade == evidence.GradeUsable {
		t.Errorf("induceCancellation(...) = %+v, want no usable grade: the runtime never honored the signal", g.obs)
	}
}
