...
func continuationRecallProfile() profile.RuntimeProfile {
	p := tokenRecognizerProfile(profile.Recognizer{
...
func assertRecallObservation(t *testing.T, recall evidence.Observation, wantGrade evidence.Grade, wantOutcome evidence.Outcome, wantDetail, wantSessionID string) {
	t.Helper()
...
func TestInduceNativeContinuationRecallEvidence(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantOutcome:   evidence.OutcomePass,
			wantDetail:    evidence.RecallSameSessionWithoutRecall,
			wantSessionID: controlSeedSessionID,
		},
		{
		...
		},
		{
			...
			wantDetail:    evidence.RecallDeclined,
			wantSessionID: controlSeedSessionID,
		},
		{
			...
			wantGrade:     evidence.GradeGap,
			wantOutcome:   evidence.OutcomePass,
			wantDetail:    evidence.RecallFreshFallback,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if seed.obs.Grade != evidence.GradeUsable || seed.obs.SessionID != controlSeedSessionID {
				t.Errorf("seed = %+v, want usable in the session %q its own output reported", seed.obs, controlSeedSessionID)
			}
			assertRecallObservation(t, recall.obs, tt.wantGrade, tt.wantOutcome, tt.wantDetail, tt.wantSessionID)
		})
	}
}
...
func TestInduceNativeContinuationReadsAStreamedSessionIdentifier(t *testing.T) {
	...
	seed, recall := induceNativeContinuation(t, Coordinates{CommandPath: script, Profile: p}, fixture, tokenTestSurface)

	if seed.obs.SessionID != "stream-session" {
		t.Errorf("seed session id = %q, want the identifier %q the launch announced outside its terminal", seed.obs.SessionID, "stream-session")
	}
	assertRecallObservation(t, recall.obs, evidence.GradeUsable, evidence.OutcomePass, evidence.RecallConfirmedSameSession, "stream-session")
}

// continuationStreamingProfile's seed prompt carries the nonce, so an
// echoing launch reaches a recall without the runtime remembering anything.
func continuationStreamingProfile() profile.RuntimeProfile {
	p := tokenRecognizerProfile(profile.Recognizer{
		Locator:       profile.TerminalLocator{Mode: "discriminated", DiscriminatorKey: "type", DiscriminatorValue: "result"},
		ErrorMembers:  []string{"error"},
		StatusMember:  "status",
	...
	entry.ResumeArgs = []string{"--resume", "{session_id}"}
	p.EntryPoints[tokenTestSurface] = entry
	p.ProbePrompts[promptKeyContinuationSeed] = controlStreamSeedPrompt
	return p
}
...
func TestInduceNativeContinuationReadsAStreamedAnswer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if seed.obs.Grade != evidence.GradeUsable || seed.obs.SessionID != sessionID {
				t.Errorf("seed = %+v, want usable in the session %q the launch announced", seed.obs, sessionID)
			}
			assertRecallObservation(t, recall.obs, tt.wantGrade, tt.wantOutcome, tt.wantDetail, sessionID)
		})
	}
}
...
func TestInduceNativeContinuationReadsOnlyTheAnsweringStream(t *testing.T) {
	...
	t.Run("a nonce on standard error does not confirm recall", func(t *testing.T) {
		assertRecallObservation(t, recall.obs, evidence.GradeGap, evidence.OutcomePass, evidence.RecallSameSessionWithoutRecall, controlSeedSessionID)
	})
...
func TestInduceProtocolContinuationAccountsForItsRecallSession(t *testing.T) {
	...
	if cleanup.Grade != evidence.GradeUsable || !strings.Contains(cleanup.Detail, "stopped_sessions=1") {
...
func continuationACPProfile() profile.RuntimeProfile {
...
func TestInduceProtocolContinuationRecallEvidence(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if seed.obs.Grade != evidence.GradeUsable || seed.obs.SessionID != controlSeedSessionID {
				t.Errorf("seed = %+v, want usable in the session %q the runtime reported", seed.obs, controlSeedSessionID)
			}
			assertRecallObservation(t, recall.obs, tt.wantGrade, tt.wantOutcome, tt.wantDetail, tt.wantSessionID)
		})
	}
