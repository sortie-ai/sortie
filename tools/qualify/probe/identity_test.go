...
func TestInduceRuntimeIdentity(t *testing.T) {
	t.Run("each handshake record supplies its own session's name and version", func(t *testing.T) {
		...
		if obs.Grade != evidence.GradeUsable || obs.Outcome != evidence.OutcomePass {
			t.Errorf("induceRuntimeIdentity() observation = %+v, want grade %q and outcome %q", obs, evidence.GradeUsable, evidence.OutcomePass)
		}
		want := map[string]evidence.SessionIdentity{
	...
	t.Run("no handshake record with a non-empty name grades not_observed with runtime_failed", func(t *testing.T) {
		...
		if obs.Grade != evidence.GradeNotObserved || obs.Outcome != evidence.OutcomeRuntimeFailed {
			t.Errorf("induceRuntimeIdentity() observation = %+v, want grade %q and outcome %q", obs, evidence.GradeNotObserved, evidence.OutcomeRuntimeFailed)
		}
		if len(identities) != 0 {
