...
// The closed set of capability-gap labels capability_gap_labels may name,
...
type DeclaredGap struct {
	Capability evidence.Capability `json:"capability"`
	Case       evidence.Case       `json:"case"`
...
type AbsentSurface struct {
	Surface evidence.Surface `json:"surface"`
	Reason  string           `json:"reason"`
}
...
func decodeDeclaredGapEntry(raw map[string]json.RawMessage) (DeclaredGap, error) {
	...
	if !slices.Contains(evidence.DeclaredGapReasons, reason) {
...
func decodeAbsentSurfaceEntry(raw map[string]json.RawMessage) (AbsentSurface, error) {
	...
	if !slices.Contains(evidence.AbsentSurfaceReasons, reason) {
...
type EntryPoint struct {
	...
	// AskingArgs is stated explicitly rather than derived from Args, since which
	// element is the posture switch is not recoverable from an argument vector.
	// Empty leaves permission handling unmeasured.
	...
	// session, with an optional "{session_id}" substituted with the seed's
	// confirmed identifier. Rejected on SurfaceProtocol.
...
type TerminalLocator struct {
	...
	// EnvelopePath descends from the located value to the terminal object, for a
	// runtime that wraps its payload below the located value. Empty is omitted
	// from the encoded form, so it never moves such a profile's digest.
...
type Recognizer struct {
	...
	StatusCases   map[string]evidence.Case `json:"status_cases"`
	...
	// SessionIDPath resolves the surface's actual session identifier for native
	// continuation. Empty falls the seed session id back to the run's generated
	// identifier and leaves the recall arm uninducible.
...
type TokenPath struct {
	...
	// Kind is "spend" or "occupancy".
	Kind string `json:"kind"`
}

// The two TokenPath.Kind values Decode admits.
const (
...
// evidence.CatalogNotInducibleCases.
type SurfaceNotInducible struct {
	Surface evidence.Surface `json:"surface"`
	Case    evidence.Case    `json:"case"`
	Reason  string           `json:"reason"` // a member of evidence.NotInducibleReasons
}
...
type RuntimeProfile struct {
	...
	// forwarded to a native launch when the parent environment carries them;
	// values never travel through this member itself.
	...
	EntryPoints map[evidence.Surface]EntryPoint `json:"entry_points"`
	Recognizers map[evidence.Surface]Recognizer `json:"recognizers"`
	...
	// [evidence.CatalogNotInducibleCases] which excludes a case everywhere.
	NotInducibleCases []SurfaceNotInducible `json:"not_inducible_cases"`
}

// The five ProbePrompts keys Decode requires.
const (
...
// Decode parses one profile document. It resolves no path.
func Decode(data []byte) (RuntimeProfile, error) {
	...
	if profile.SchemaVersion, err = evidence.DecodeInt(top["schema_version"]); err != nil {
	...
	if profile.RuntimeID, err = evidence.DecodeString(top["runtime_id"]); err != nil {
	...
	if profile.NotesPath, err = evidence.DecodeString(top["notes_path"]); err != nil {
		return RuntimeProfile{}, fmt.Errorf("notes_path: %w", err)
	}
	if profile.MeasurementPath, err = evidence.DecodeString(top["measurement_path"]); err != nil {
		return RuntimeProfile{}, fmt.Errorf("measurement_path: %w", err)
	}
	if profile.PublishedSample, err = evidence.DecodeString(top["published_sample"]); err != nil {
	...
	if profile.ToolNameFormat, err = evidence.DecodeString(top["tool_name_format"]); err != nil {
	...
	if toolPolicyFormatRaw, has := top["tool_policy_format"]; has {
		if profile.ToolPolicyFormat, err = evidence.DecodeString(toolPolicyFormatRaw); err != nil {
	...
	if _, hasProtocol := profile.EntryPoints[evidence.SurfaceProtocol]; !hasProtocol {
		return RuntimeProfile{}, fmt.Errorf("entry_points must carry %q", evidence.SurfaceProtocol)
	}
	...
	// The evidence fixture and the summary size their baseline sets from two
	// independent rules; a surface left out of both would surface only as a
	// downstream baseline-count mismatch.
...
func checkContinuationCoordinates(entryPoints map[evidence.Surface]EntryPoint, recognizers map[evidence.Surface]Recognizer) error {
...
// checkMeasuredEntryPointShape requires asking_args on every measured surface,
// and "{prompt}" exactly once in args and asking_args on a native surface but
// never on the protocol surface.
func checkMeasuredEntryPointShape(profile RuntimeProfile) error {
	for _, surface := range profile.MeasuredSurfaces() {
		...
		wantPrompt := surface != evidence.SurfaceProtocol
...
// describe.
var notInducibleReasonCase = map[string]evidence.Case{
	evidence.NotInducibleChannelTooSmall:          evidence.CaseLimitReached,
	evidence.NotInducibleOutputSilentOnFailure:    evidence.CaseRuntimeFailure,
	evidence.NotInducibleTerminalAtExitOnly:       evidence.CaseCancellation,
	evidence.NotInducibleTerminalVocabularyClosed: evidence.CaseHumanInput,
}

// decodeNotInducibleCases strictly decodes the not_inducible_cases member. The
...
func decodeNotInducibleCases(raw json.RawMessage, profile RuntimeProfile) ([]SurfaceNotInducible, error) {
	...
	for i, fields := range rawEntries {
		if err := evidence.CheckObjectFields(fields, surfaceNotInducibleFields); err != nil {
	...
	for i, entry := range entries {
		if !slices.Contains(evidence.Surfaces, entry.Surface) || entry.Surface == evidence.SurfaceAggregate {
			return nil, fmt.Errorf("[%d]: surface %q is outside the measured surface set", i, entry.Surface)
		}
		if !slices.Contains(evidence.Cases, entry.Case) {
			return nil, fmt.Errorf("[%d]: case %q is outside evidence.Cases", i, entry.Case)
		}
		if slices.Contains(evidence.CatalogNotInducibleCases, entry.Case) {
			return nil, fmt.Errorf("[%d]: case %q is already catalog-wide not-inducible", i, entry.Case)
		}
		if !slices.Contains(evidence.NotInducibleReasons, entry.Reason) {
		...
		seen[key] = true
		if _, declared := profile.Declared(evidence.CapabilityOwning(entry.Case), entry.Case); declared {
...
// CaseExclusion reports what one surface's silence on caseID means. A
...
func (p RuntimeProfile) CaseExclusion(surface evidence.Surface, capability evidence.Capability, caseID evidence.Case) evidence.ExclusionKind {
	...
	if reason, ok := p.NotInducibleDeclared(surface, caseID); ok {
		return evidence.NotInducibleExclusion(reason)
	}
	return evidence.ExclusionNone
}
...
func decodeEntryPoints(raw json.RawMessage) (map[evidence.Surface]EntryPoint, error) {
	var rawEntries map[string]map[string]json.RawMessage
	if err := json.Unmarshal(raw, &rawEntries); err != nil {
		return nil, err
	}
	entries := make(map[evidence.Surface]EntryPoint, len(rawEntries))
	for key, fields := range rawEntries {
		surface := evidence.Surface(key)
		if !slices.Contains(evidence.Surfaces, surface) {
			return nil, fmt.Errorf("%q is outside evidence.Surfaces", key)
		}
		if surface == evidence.SurfaceAggregate {
...
func structuredNativeSurfaces(entryPoints map[evidence.Surface]EntryPoint) []evidence.Surface {
	var surfaces []evidence.Surface
	for surface := range entryPoints {
		if surface == evidence.SurfaceProtocol || surface == evidence.SurfaceAggregate {
			continue
		}
...
// entryPoints.
func decodeRecognizers(raw json.RawMessage, entryPoints map[evidence.Surface]EntryPoint) (map[evidence.Surface]Recognizer, error) {
	var rawEntries map[string]map[string]json.RawMessage
	if err := json.Unmarshal(raw, &rawEntries); err != nil {
		return nil, err
	}
	...
	recognizers := make(map[evidence.Surface]Recognizer, len(rawEntries))
	for key, fields := range rawEntries {
		surface := evidence.Surface(key)
		if surface == evidence.SurfaceProtocol {
...
func decodeRecognizerEntry(fields map[string]json.RawMessage) (Recognizer, error) {
	...
	var rawStatusCases map[string]evidence.Case
	...
	for status, caseID := range rawStatusCases {
		if !slices.Contains(evidence.Cases, caseID) {
			return Recognizer{}, fmt.Errorf("status_cases[%q] = %q is outside evidence.Cases", status, caseID)
		}
	}
	...
	if tokenPathsRaw, has := fields["token_paths"]; has {
		...
		for i, pathFields := range rawPaths {
			if err := evidence.CheckObjectFields(pathFields, tokenPathFields); err != nil {
...
// decodeStringOrEmpty only rejects a wrong-typed value: the caller's
// field-presence check already rejected an absent member, and an explicit
// empty string is a valid "this member is unused" state.
...
func decodeDeclarationFields(top map[string]json.RawMessage) ([]DeclaredGap, []AbsentSurface, error) {
	...
	for i, entry := range declarations {
		peer, hasPeer := evidence.DeclaredGapPeers[entry.Case]
		if !hasPeer {
			continue
		}
		peerKey := [2]string{string(evidence.CapabilityOwning(peer)), string(peer)}
	...
	seenAbsent := map[evidence.Surface]bool{}
...
// Load reads path, decodes it, and resolves notes_path, measurement_path
// and published_sample against root. It never derives root itself.
func Load(root, path string) (RuntimeProfile, error) {
...
func (p RuntimeProfile) Declared(capability evidence.Capability, caseID evidence.Case) (string, bool) {
...
// pair.
func (p RuntimeProfile) NotInducibleDeclared(surface evidence.Surface, caseID evidence.Case) (string, bool) {
...
func (p RuntimeProfile) AbsentSurfaceDeclared(surface evidence.Surface) (string, bool) {
...
// MeasuredSurfaces returns p.EntryPoints's keys minus every surface p declares
// absent: a declared-absent surface keeps its entry point but writes no
// record, so including it would over-count a caller sizing an expected
// record set.
func (p RuntimeProfile) MeasuredSurfaces() []evidence.Surface {
	var surfaces []evidence.Surface
...
// DeclaredMeasuredSurfaces returns the closed measurable order minus every
// surface the profile declares absent; the zero RuntimeProfile returns all
// three. Unlike [RuntimeProfile.MeasuredSurfaces], which reads a live
// profile's entry_points, an offline validator and this package's test
// fixtures work from a profile's declarations alone.
func (p RuntimeProfile) DeclaredMeasuredSurfaces() []evidence.Surface {
	surfaces := make([]evidence.Surface, 0, len(evidence.MeasurableSurfaces()))
	for _, surface := range evidence.MeasurableSurfaces() {
		if _, absent := p.AbsentSurfaceDeclared(surface); !absent {
			surfaces = append(surfaces, surface)
		}
...
func (p RuntimeProfile) EntryArgs(surface evidence.Surface, model, policy, prompt string) ([]string, error) {
...
// EntryPoint.AskingArgs. It reports false when the profile states no asking
// posture, the caller's signal to record the row unmeasured.
func (p RuntimeProfile) AskingArgs(surface evidence.Surface, model, policy, prompt string) ([]string, bool) {
	entry, ok := p.EntryPoints[surface]
...
// values, falling back to a streaming decode when no line decodes on its own:
// a native surface's output is not necessarily one JSON document.
...
// SessionID resolves r.SessionIDPath from output. A streaming surface
// announces its session in its first event and need not repeat it in the
// terminal, so this also checks other top-level values when the terminal
// does not resolve it.
...
// Terminal recognizes one native surface's terminal outcome from its raw
// output. It reports false when no terminal object is located or none of its
// members resolve to a known outcome.
...
// letting a caller distinguish "no envelope exists" from "an envelope exists
// but none of its members resolved to a known outcome", a distinction
// [Recognizer.Terminal] discards once recognition fails.
...
// ModelRequests reads r.ModelRequestPath from output, without reading message
// text. An unreadable object is reported rather than defaulted to
// ModelRequestNone, which would manufacture a false positive out of a
// truncated terminal.
