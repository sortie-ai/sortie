...
Sortie 是一个单一 Go 二进制文件。它使用 SQLite 存储持久化状态（重试队列、会话元数据、运行历史），并通过 stdio 与编程智能体通信。编排器是所有调度决策的唯一权威；不依赖外部任务队列或分布式协调。该二进制文件不发送遥测数据：没有分析客户端，没有更新检查，它访问的端点仅限于你在工作流文件中声明的那些。完整架构详情参见 [`docs/architecture.md`](docs/architecture.md)。
