...
Exit zero does not mean the turn succeeded on the text path. The cost trailer on standard error is the only positive proof that a turn actually ran. A turn that never ran because the credential was rejected also exits zero, with empty stdout and an authentication line on stderr and no trailer. A turn whose standard error could not be collected has no trailer to read either, and is reported as a failure for the same reason.
