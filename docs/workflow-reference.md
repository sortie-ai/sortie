...
| `command`                        | string (argument vector)          | When adapter requires local process | Adapter-defined | Future dispatches                          | Command to launch the agent for adapters that run as a local subprocess (`claude-code`, `copilot-cli`, `codex`, `opencode`, and `kiro`). Adapters that do not start a local process ignore this field. |
...
| `agent.command`                         | argument vector  | adapter-defined              | `SORTIE_AGENT_COMMAND`                   | Required for local adapters                                                            |
