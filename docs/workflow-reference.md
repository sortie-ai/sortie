...
| --- | --- | --- |
| `KIRO_API_KEY` | No, alongside a stored login | Headless credential. Requires a Kiro Pro, Pro+, or Power subscription. Either this key or a stored `kiro-cli` login satisfies the credential-verification step (Section 2.6): its `kiro-cli whoami` guard runs only on that step's own session, deciding on the exit status alone, because headless `chat` with no credential blocks on an interactive device-login flow with no self-timeout; see [Section 4.3](#43-worker--ssh-worker-extension) for how the key reaches a remote session. |
