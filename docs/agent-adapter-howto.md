...
3. **Update the adapter notes file to match the fresh measurement.** Rewrite it in the present tense to describe current behavior; do not append a dated changelog entry describing what changed. Where the runtime is qualification-profiled, the notes document's required sections, its one `Eligibility:` line carrying the transport-parity verdict, its `Product conformance:` line where the run answered that question, and its per-surface-capability status rows must match the new `EligibilityReport` exactly. Do not write a conformance line into a document whose run produced none: `qualification.ValidateNotes` rejects a verdict no measurement stands behind. Validate the rewritten document against the fresh expectation with `qualification.ValidateNotes` before treating the refresh as done; a mismatched grade row or a missing section is a defect in the notes, not an acceptable rounding of the measurement.

4. **Record the fresh measurement artifact.** Where the runtime carries a tracked measurement document, replace it with one built from this run's `NotesExpectation`, which carries both verdicts, bound to the profile document's own digest (`RuntimeProfile.Digest`) so a later edit to the profile without a fresh run is detectable rather than silently treated as still current. Carry the run's provenance digest with it: a profile digest and a date are shared by two runs of different collector builds, so they do not identify which build measured this.
