...
The wire version and the schema release are different numbers, and conflating them is a known trap: the publisher versions its schema artifacts on their own line and states outright that wire compatibility must not be inferred from it. Validation and the protocol types are therefore pinned to one named stable schema release, recorded with the tag and the commit it resolves to, and that pin moves deliberately rather than by following the latest publication. The release pinned at adoption is the stable schema artifact published under the tag schema-v1.21.0, which is an annotated tag resolving to commit 272bf799f35a258c6a4107a0410ed361e83683d3. It is the stable artifact, not the unstable one published beside it under the same tag, and its own metadata declares wire version 1. Naming the release alone would not identify it, because the tag and the wire version are different numbers and the stable and unstable artifacts ship together.
...
**Taking the most-starred Go module.** Rejected, and not for being a dependency. It needs no C toolchain, it builds statically, and it adds one entry to the module graph, so the project's one hard prohibition is not what decides this. Three measured properties are. Its vendored schema is the release published 80 days before the artifact pinned here, and its build recipe fetches schema assets from a tag namespace the publisher has stopped using, so it cannot reach this pin without work done here; contributed changes offering both the namespace fix and the intermediate bump have sat unanswered for nineteen days in a repository whose last maintainer commit is three months old, while that maintainer's public work continued elsewhere on the day of measurement. Its client contract is one Go interface of nine methods, and a Go interface has no optional members: two are the permission request and the session update this transport needs, and seven are the filesystem and terminal operations it refuses to advertise. Against the pinned artifact that surface is eleven, so catching up to the pin would widen the refused set to nine rather than narrow it. And its decoder contradicts the parsing posture settled below: an unknown session-update variant is accepted without error and leaves every variant field empty, so the unrecognized part is dropped rather than recorded.

**Forking it.** Rejected, and not as a hypothetical. Three consumers of that module have already routed around the same layer inside it. Its largest consumer maintains a fork, a second contributor maintains another, and a third builds its connection through a hand-written router because the module's own dispatcher answers method-not-found to any vendor method that does not carry an underscore prefix, which real agents send. That layer is framing and dispatch, which this project already owns and already runs in production. The measured fork is one commit across thirty-five files that rewrites the generator, regenerates every type, and adds a legacy-compatibility file, and it still lands one schema release short of this pin. What it demonstrates is the definition already given: a hand-written implementation carrying a standing obligation to reconcile with an upstream that is not moving.

**Taking the one module that tracks this pin.** Rejected on evidence that cuts both ways, and the favorable half is recorded because it corrects a claim a later reader would otherwise inherit. Its lock file records the same tag, tag object, and commit pinned here, together with digests for both assets, and those digests match the published assets exactly. It also splits its client contract so that only the permission request and the session update are required, with every refused capability behind a separate optional interface. That establishes that nine methods with seven empty bodies is a property of one module's shape and not a cost of depending on anything. What rejects it is the posture that also rejects the first: it fails an unknown session-update variant outright, which every runtime measured for this decision would trigger. And it has no operating history. Its organization is five days older than its first release, and a search of published Go module files finds it imported only inside that organization, so there is no evidence in either direction about whether it will be maintained.

**Writing the types by hand.** Rejected as a steady state rather than as a start. The pinned artifact carries 170 definitions, and re-deriving them by hand on every pin move puts the cost of moving the pin on a person, which makes the pin move less often than the evidence warrants.

The rejected routes share the reason the chosen one wins. What an external module genuinely supplies is a set of types, and both measured modules supply them against a decoding posture this record rejects: the pinned schema carries the marker requiring a default to be substituted rather than the message failed 249 times, and neither module's generated code honors it anywhere. What would be worth taking is the framing and dispatch beneath the types, and that is the half every measured consumer replaced and the half this project already has. Generating from the artifact takes the part that has held still, the schema, and declines the part that has not, another project's opinion about how to decode it.

One cost is unavoidable and is offset nowhere else. A general-purpose schema-to-type generator does not work on this schema: the current release of the widely used one produces no output file at all, on an input where a trivial control schema generates cleanly, because the schema wraps references in single-element compositions, leaves its unions untagged, and expresses enumerations as string constants rather than as enumerations. A generator is therefore written here. It is written on the fork route too, and both measured modules wrote their own, so only adopting a module unchanged avoids it, and that is rejected above on other grounds.

Reconsideration is triggered by a module meeting three conditions at once rather than by a module becoming popular: it tracks the schema release this project pins, its generated code honors the default-on-error marker, and it has enough operating history that its maintenance is a fact rather than a forecast. No module measured for this decision meets more than one.
...
- The pin is verifiable by digest and moves on this project's schedule. The schema and its method index are published as downloadable assets, so the input to type generation is fixed by content and a moved tag cannot change it unnoticed. Every measured Go consumer of the leading community module sits behind the current stable schema, one of them by ten months, because their currency is set by that module's release cadence rather than by their own.
...
- **The client is not small, and a general-purpose generator does not produce it.** Closing over the messages the transport needs reaches roughly three to five times the declared type surface of the project's existing persistent-session integration, most of it in unions the implementation language does not express directly. A general schema-to-type generator was tried against four releases of the published schema and produced no output at all on any of them, so a generator built for this schema is part of this work rather than a tool taken off a shelf, and that work appears in no count of types. Both measured community modules wrote their own for the same reason.
- **Owning the types means meeting their defects first.** The most-starred community module carries operational exposure this project will not have: its import path appears in 139 published Go module files in a search that indexes default branches only, so its bugs are found by other people, and two concurrency defects in it were reported by third parties in the four days before this record. A client written here has no such population. The gated conformance test named above is the only substitute for it, which raises that test from useful to load-bearing.
...
11. The protocol types are reproducible from the pinned schema artifact rather than maintained by hand: the generator runs against the recorded tag and commit, the assets it consumes are verified against their recorded digests before generation, and regenerating produces no difference from the types in the tree.
