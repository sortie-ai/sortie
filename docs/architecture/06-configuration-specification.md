...
- The per-issue token ceiling (`agent.max_tokens`) is not a scheduler preflight check. It is
  evaluated on three lanes: the retry path and the poll tick's own rebuild block a re-dispatch,
  alongside `agent.max_sessions` (Section 8.4), and the event loop stops a run already in
  flight as each usage figure arrives, so the ceiling bounds a blocked issue and a running
  session rather than failing startup or a poll tick. Config-level validation rejects a
