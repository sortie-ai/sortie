...
A scheduled check reports when the publisher's stable schema releases, or its own copy of the pinned release, differ from this pin. It never moves the pin, and it closes its report on the first run that finds no difference of any kind.
...
The registered kind declares `turn_end` arrival and `per_model` attribution, with one session rule resolving a remote launch to `none`/`none`. The figure does not come off the wire: `normalize.go`'s `updateUsage` still folds a `usage_update` session update into the debug-log-only arm of the normalization table, and the `_meta.quota` block a prompt result may carry is read as a presence signal and a lower bound only, never as the accounting figure.

The figure comes from a measurement source outside the protocol, selected once when the session is built and held as an interface value the transport never asks the identity of. Selection passes three gates, each of which can only lower the `tokenCounts` capability entry: a source claims the resolved launch target before launch and takes the `ours` state, the handshake lowers it to `gap` when the runtime's own `agentInfo` names a build the source was not measured against, and the first drain that produces no record lowers it to `gap` for the rest of the session. A session with no applicable source, which includes every remote launch, is reported unmeasured with the token ceiling inactive, exactly as before.

One source exists, under `internal/agent/clientprotocol/usagesource`. It reads `@google/gemini-cli` 0.59.0's own OpenTelemetry output as the primary source and that runtime's session journal as the durability backstop, mapping `input_token_count` to input (the cache read is already inside it), `output_token_count` plus `thoughts_token_count` to output, and `cached_content_token_count` to the cache-read subset; the runtime's own total and its `tool_token_count` are discarded. The wait for a record is bounded and ends early against the wire extension's own lower bound, because the source is batched behind a five second tick no configuration shortens. A turn that reached the model and then ended without a prompt result of its own, on a stream end, a write failure, an error response or a result that did not decode, still gets one bounded wait; it carries no lower bound to end that wait early, and whatever it recovers changes nothing about the disposition the turn is reported on.

A turn that issued a model request no figure was proven to account for is recorded as spend occurred, part of the amount unknown: a turn with no figure at all contributes no number, a turn whose figure fell short of it contributes that figure and leaves the rest out, either way the issue's summed total becomes a lower bound, and both are distinct from a measured zero and from the measurement verdict beside them. Every turn of a remote launch that reports spend on the wire reaches that state, because nothing reads a far host's filesystem; which mechanism would carry a remote session's measurements back is undecided rather than unavailable.
...
The credential-verification step's `initialize` wait is the larger of `agent.read_timeout_ms` and `agentcore.CredentialExchangeBound` (60 seconds): a runtime may need to establish its credential with its own backend before it answers at session start at all. Measured against `kiro-cli acp`: under a refused API key, `initialize` answered 16.1 to 19.3 seconds after start, `session/new` a further 1.8 to 2.9 seconds; under a valid key, `initialize` and `session/new` finished 17.1 and 17.6 seconds after start; under a stored device login, the same handshake finished in 0.12 to 0.29 seconds. At the 5-second default `read_timeout_ms`, either key would fail the handshake with `response_timeout` before the runtime ever answers; given the longer wait, a refused key instead fails its first prompt with a `-32603` error whose `data` names the invalid bearer token.

A `-32000` response to `initialize` or `session/new`, from any caller, is read as the runtime refusing the credential it was given, on a verification session and a working one alike. A runtime that exits before any startup request is answered, on a verification session and a working one alike, fails with the shared early-exit report rather than a credential diagnosis. A connection lost while the runtime stays alive past the observation grace keeps `port_exit`'s own transport message. `session/load` and `session/resume` keep their own fallback behavior unchanged: an answer other than success there still falls back to a fresh session rather than being read as a credential refusal.

No installed protocol runtime measured for these notes advertises the pinned schema's `sessionCapabilities.delete`. Where a handshake does advertise it, a verification session's teardown sends `session/delete` in place of `session/close`, so the runtime is asked to drop the verification's own conversation rather than leaving it in whatever store the runtime keeps; a runtime that advertises neither capability leaves that conversation in place.
...
Unit tests cover wire parsing, event normalization, permission replies, capability lowering and session continuation, and they need no credential. The tests that drive a real binary are env-gated on `SORTIE_CLIENTPROTOCOL_TEST=1` and skip cleanly without it; keep them skipping cleanly rather than failing. Because this kind names no default runtime, `SORTIE_CLIENTPROTOCOL_COMMAND` supplies the launch command together with whatever flag or subcommand puts that binary into protocol mode, and its absence skips the suite exactly as a missing credential does in the sibling agent suites. The binary needs a working credential in the environment. `SORTIE_CLIENTPROTOCOL_CREDENTIAL_ENV` names the environment variables the refused-credential case of the credential-verification suite sets to an invalid value; its absence skips that one case cleanly, with a logged reason, while the rest of the gated suite still runs. The gated suite also runs the configured command with `--sortie-unknown-switch` appended, on a verification session and a working session alike, proving the run fails before any work with the runtime's own exit status and standard error rather than a lost connection or a credential problem; a runtime that accepts the switch skips that case with a logged reason naming it.

A separate, Unix-only qualification profile drives a live model across several surfaces for minutes to decide whether a runtime is eligible for this transport. It is gated on `SORTIE_CLIENTPROTOCOL_QUALIFICATION_TEST=1` and skips cleanly without it. Once enabled it reads four coordinates: `SORTIE_CLIENTPROTOCOL_QUALIFICATION_COMMAND` names exactly one executable path with no flags, because the profile appends each surface's own; `SORTIE_CLIENTPROTOCOL_QUALIFICATION_MODEL` names the one model identifier applied to every surface; `SORTIE_CLIENTPROTOCOL_QUALIFICATION_AUTH_ENV_NAMES` lists the authentication environment variable names the profile forwards, names only, never values, and may be set to an empty list for a runtime that authenticates from a stored login instead of from the environment, which is a declaration rather than an omission: the coordinate must still be set, so forgetting it fails the run exactly as before; and `SORTIE_CLIENTPROTOCOL_QUALIFICATION_PROFILE` names one readable runtime profile document carrying the runtime's entry points, native recognizers, and declared gaps together, renamed from the coordinate this replaces. It is now REQUIRED: its absence fails the run rather than resolving to an empty declaration set, because the run's entry points live there. A profile may declare a capability gap, a surface the runtime offers no entry point for, or both; declaring a surface absent is corroborated by its own launch before any probe spends anything else, so a false declaration stops the run rather than being trusted. An enabled gate with any coordinate missing or invalid fails the run, except that a named authentication variable carrying no value is a clean skip rather than a failure, per the credential paragraph below.

`SORTIE_CLIENTPROTOCOL_QUALIFICATION_AUTH_ENV_NAMES` naming a variable that carries no value in the invoking environment is a clean skip, not a run failure: the operator mints a temporary credential for the run rather than spending a long-lived one, and an unsupplied one just means that credential has not been minted yet. Every other coordinate, and an `AUTH_ENV_NAMES` entry that is itself absent or malformed, still fails the run rather than skipping it.

`SORTIE_CLIENTPROTOCOL_QUALIFICATION_MODEL` names the cheapest model passing three rules: it is one the runtime's own model-listing command offers; it supports every capability the collection's own load-bearing rows depend on (tool calls, literal-token and nonce reproduction, turn refusal and its report, reading prior turns within one session, and a context window below the oversize-filler ceiling); and no case reaches its own not-observed arm on every measured surface with no failure outcome and no declaration, which disqualifies the model rather than justifying a declaration. A runtime's first bring-up drives roughly thirty paid turns per pass under this model coordinate, and the run records the selected coordinate verbatim in the measurement's `model` field.

The live capture and its offline evaluation live in separate places since the qualification tooling moved into its own module, `tools/qualify`, `replace`-bound to the checkout so it always measures local code; no root-module package imports it, and it ships in no release artifact. `make qualify-live` drives the gated live capture above; `make qualify-report CAPTURE_DIR=<path>` and `make qualify-reemit CAPTURE_DIR=<path>` re-derive a report or re-emit `measurement.json`/`provenance.json` from an existing capture directory offline, needing no runtime, credential, or network. Both read `SORTIE_CLIENTPROTOCOL_QUALIFICATION_CAPTURE_DIR`: unset, it defaults to the tracked capture corpus inside the tool module and the run MUST NOT skip; set to a path that is not a readable capture directory, the run fails rather than skipping, so a typo cannot pass green.
...
One rule asserts a model choice rather than a protocol shape: that the prompt written to force a tool call produces a `tool_call` update and a terminal `tool_call_update`. Three consecutive live runs each carried the pair, with the tool call's own identifier picked up by the terminal update every time rather than the two counts merely both being non-zero. That rule does not gate the shard: each run records it as a `model-dependent observation: `-prefixed log entry, because the protocol obligates the agent to report a tool call the model requests, never the model to request one. The rule that does gate the shard, S-9, keeps failing whenever a tool call occurs without a normalized `domain.EventToolResult`, so a normalization regression on the tool path still reddens the suite. A runtime that stops emitting tool calls entirely shows up in that per-run observation log rather than as an escalated adapter regression.
