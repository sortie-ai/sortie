...
Every runtime that launches locally as a subprocess fills the same three command-line slots: one that puts the runtime on the protocol wire, one that grants the workspace the trust a declared tool server's delivery depends on, and one that puts the runtime in a posture that does not ask before running a tool. `gemini`'s measured values fill them below; a second runtime fills the same three slots with its own values, and no shared code branches on which runtime is naming them.
