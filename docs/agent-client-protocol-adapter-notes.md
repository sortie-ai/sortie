...
A `-32000` response to `initialize` or `session/new`, from any caller, is read as the runtime refusing the credential it was given, on a verification session and a working one alike. A connection that ends before `initialize` answers, on a verification session only and other than an SSH connection failure, is read as the credential step not completing, distinct from a working session's own connection loss. `session/load` and `session/resume` keep their own fallback behavior unchanged: an answer other than success there still falls back to a fresh session rather than being read as a credential refusal.
