...
A separate, Unix-only qualification profile drives a live model across several surfaces for minutes to decide whether a runtime is eligible for this transport. It is gated on `SORTIE_CLIENTPROTOCOL_QUALIFICATION_TEST=1` and skips cleanly without it. Once enabled it reads three coordinates: `SORTIE_CLIENTPROTOCOL_QUALIFICATION_COMMAND` names exactly one executable path with no flags, because the profile appends each surface's own; `SORTIE_CLIENTPROTOCOL_QUALIFICATION_MODEL` names the one model identifier applied to every surface; and `SORTIE_CLIENTPROTOCOL_QUALIFICATION_AUTH_ENV_NAMES` lists the authentication environment variable names the profile forwards, names only, never values. An enabled gate with a coordinate missing or invalid fails the run rather than skipping it. Neither the release workflow nor the nightly workflow runs this profile.
