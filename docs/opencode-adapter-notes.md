...
- Which failures other than an unknown model arrive as the masked generic server error. The adapter reconstructs the diagnostic for the unknown-model case alone, and no probe enumerates the rest, so any other masked cause reaches the operator as the placeholder. Probe an invalid model, a revoked credential, and an aborted message, and compare the reported detail in each.
