...
The adapter owns both pipe read ends for the turn's lifetime, so the reap runs independently of the stdout reader: reaping the process no longer closes either read end, and the reader keeps draining whatever the process wrote regardless of when the reap happens. The stderr bound inside the wait goroutine is what still protects the permission-refusal warning the finalize path depends on: it is bounded so a descendant that inherits only the stderr handle and outlives the direct child cannot withhold the reap, and firing that bound costs the warning on the residual path where reaping the process does not also release the collector. The four early-return arms bound their own drain of the stdout reader rather than waiting on it unconditionally, so a descendant that inherits only the stdout handle cannot withhold any of them past that same bound.
