...
- Responses carry an RFC 8288 `Link` header (`rel="next"`, `rel="last"`) with absolute URLs, plus `X-Total-Count` (verified; same shape the GitHub adapter consumes). `httpkit.NewLinkPaginator` and `httpkit.ParseLinkRel` handle it unchanged; the paginator follows the full `Link` URL, which is correct here because Gitea's next-page URLs preserve the query parameters (verified).
